...
//  Blocstagram
//
//  Created by Casey Ward on 5/1/15.
...
- (void) addFiltersToQueue {
    ...
    //Rad filter
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *radFilter = [CIFilter filterWithName:@"CIPhotoEffectInstant"];
        
        if (radFilter) {
            [radFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            [self addCIImageToCollectionView:radFilter.outputImage withFilterTitle:NSLocalizedString(@"Rad", @"Rad Filter")];
        }
    }];
    ...
    [self.photoFilterOperationQueue addOperationWithBlock:^{
        CIFilter *radFilter = [CIFilter filterWithName:@"CIPhotoEffectInstant"];
        CIFilter *secondFilter = [CIFilter filterWithName:@"CIPhotoEffectChrome"];
        
        if (radFilter) {
            [radFilter setValue:sourceCIImage forKey:kCIInputImageKey];
            CIImage *result =radFilter.outputImage;
            NSLog(@"rad filter was made");
            
            if (secondFilter) {
                [secondFilter setValue:result forKey:kCIInputImageKey];
                result = secondFilter.outputImage;
                NSLog(@"second filter was made");
            }
            NSLog(@"extra rad filter was made");
            [self addCIImageToCollectionView:result withFilterTitle:NSLocalizedString(@"Extra Rad", @"Extra Rad Filter")];
        }
    }];
...
#pragma mark - Posting to Instagram
